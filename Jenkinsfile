...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3636
        NAMESPACE = "snapmoment-org"
        REGISTRY_HOSTNAME = "momecap"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "snapmoment-admin"
        DEPLOYMENT_NAME = "snapmoment-admin-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'snapmoment-org') {
